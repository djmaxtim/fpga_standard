// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/blanc/FIFO36E1.v,v 1.33.4.1 2011/05/27 18:20:22 wloo Exp $
///////////////////////////////////////////////////////////////////////////////
...
module FIFO36E1 (ALMOSTEMPTY, ALMOSTFULL, DBITERR, DO, DOP, ECCPARITY, EMPTY, FULL, RDCOUNT, RDERR, SBITERR, WRCOUNT, WRERR,
	...
	(RDCLK => SBITERR) = (100, 100);
        (WRCLK *> ECCPARITY) = (100, 100);

	(RDCLK => ALMOSTEMPTY) = (100, 100);
	(RDCLK => EMPTY) = (100, 100);
	(RDCLK *> RDCOUNT) = (100, 100);
	(RDCLK => RDERR) = (100, 100);

	(WRCLK => ALMOSTFULL) = (100, 100);
	(WRCLK => FULL) = (100, 100);
	(WRCLK *> WRCOUNT) = (100, 100);
	(WRCLK => WRERR) = (100, 100);
		...
		if (period_rdclk == period_wrclk) begin
			...
			if (SIM_DEVICE == "7SERIES") begin
				...
				if ((ALMOST_EMPTY_OFFSET < 5) || (ALMOST_EMPTY_OFFSET > addr_limit - 6)) begin
				    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on FF36_INTERNAL_VLOG instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 5, addr_limit - 6);
				    $finish;
				end
				
				if ((ALMOST_FULL_OFFSET < 4) || (ALMOST_FULL_OFFSET > addr_limit - 7)) begin
				    $display("Attribute Syntax Error : The attribute ALMOST_FULL_OFFSET on FF36_INTERNAL_VLOG instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_FULL_OFFSET, 4, addr_limit - 7);
				    $finish;
				end
				...
				if ((ALMOST_EMPTY_OFFSET < 6) || (ALMOST_EMPTY_OFFSET > addr_limit - 5)) begin
				    $display("Attribute Syntax Error : The attribute ALMOST_EMPTY_OFFSET on FF36_INTERNAL_VLOG instance %m is set to %d.  Legal values for this attribute are %d to %d", ALMOST_EMPTY_OFFSET, 6, addr_limit - 5);
				    $finish;
				end
			...
			if (ALMOST_FULL_OFFSET > (addr_limit - ((4 * roundup_int_period_rdclk_wrclk) + 6))) begin
			    ...
			    $display("DRC Error : The attribute ALMOST_FULL_OFFSET on FF36_INTERNAL_VLOG instance %m is set to %d. It must be set to a value smaller than (FIFO_DEPTH - ((4 * roundup (WRCLK frequency / RDCLK frequency)) + 6)) when FF36_INTERNAL_VLOG has different frequencies for RDCLK and WRCLK.", ALMOST_FULL_OFFSET);
			    $finish;
			...
			if (ALMOST_FULL_OFFSET > (addr_limit - ((3 * roundup_int_period_wrclk_rdclk) + 3))) begin
			    ...
			    $display("DRC Error : The attribute ALMOST_FULL_OFFSET on FF36_INTERNAL_VLOG instance %m is set to %d. It must be set to a value smaller than (FIFO_DEPTH - ((3 * roundup (RDCLK frequency / WRCLK frequency)) + 3)) when FF36_INTERNAL_VLOG has different frequencies for RDCLK and WRCLK.", ALMOST_FULL_OFFSET);
			    $finish;
			...
			if (ALMOST_EMPTY_OFFSET > (addr_limit - ((3 * roundup_int_period_rdclk_wrclk) + 3))) begin
			    ...
			    $display("DRC Error : The attribute ALMOST_EMPTY_OFFSET on FF36_INTERNAL_VLOG instance %m is set to %d. It must be set to a value smaller than (FIFO_DEPTH - ((3 * roundup (WRCLK frequency / RDCLK frequency)) + 3)) when FF36_INTERNAL_VLOG has different frequencies for RDCLK and WRCLK.", ALMOST_EMPTY_OFFSET);
			    $finish;

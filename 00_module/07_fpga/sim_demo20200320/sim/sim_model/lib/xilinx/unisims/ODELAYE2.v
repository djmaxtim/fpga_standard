...
//------------------------------------------------------------
    ...
    initial begin
        ...
        if (REFCLK_FREQUENCY < MIN_REFCLK_FREQUENCYL || REFCLK_FREQUENCY > MAX_REFCLK_FREQUENCYH) begin
            $display("Attribute Syntax Error : The attribute REFCLK_FREQUENCY on ODELAYE2 instance %m is set to %f.  Legal values for this attribute are 175.0 to 225.0", REFCLK_FREQUENCY);
            $finish;
...
//*********************************************************
    ...
    specify
        ( C *> CNTVALUEOUT) = (100, 100);
        ( C => DATAOUT) = (100, 100);
        ( CINVCTRL *> CNTVALUEOUT) = (0, 0);

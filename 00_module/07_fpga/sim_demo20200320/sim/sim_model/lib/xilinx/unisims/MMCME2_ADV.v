// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/fuji/MMCME2_ADV.v,v 1.13.8.2 2011/06/10 18:56:41 yanx Exp $
///////////////////////////////////////////////////////////////////////////////
...
//      "TRUE" : clkin_hold_f = 1;
//      default : begin
//        $display("Attribute Syntax Error : The Attribute CLOCK_HOLD on MMCME2_ADV instance %m is set to %s.  Legal values for this attribute are TRUE or FALSE.", CLOCK_HOLD);
//        $finish;
//      end
//    endcase
...
//        $display("Attribute Syntax Error : The Attribute CLKFBOUT_USE_FINE_PS on MMCME2_ADV instance %m is set to %s.  This attribute should be set to FALSE when CLKFBOUT_MULT_F has fraction part.", CLKFBOUT_USE_FINE_PS);
//        $finish;
//      end
//    else
        clkfb_fps_en = 1;
...
//        $display("Attribute Syntax Error : The Attribute CLKOUT0_USE_FINE_PS on MMCME2_ADV instance %m is set to %s.  This attribute should be set to FALSE when CLKOUT0_DIVIDE has fraction part.", CLKOUT0_USE_FINE_PS);
//        $finish;
//      end
 //     else
        clk0_fps_en = 1;
...
//        $display("Attribute Syntax Error : The Attribute CLKOUT0_PHASE on MMCME2_ADV instance %m is set to %f.  This attribute should be set to 0.0 when CLKOUT0_DIVIDE_F has fraction part.", CLKOUT0_PHASE);
//        $finish;
//       end
...
//        $display("Attribute Syntax Error : The Attribute CLKFBOUT_PHASE on MMCME2_ADV instance %m is set to %f.  This attribute should be set to 0.0 when CLKFBOUT_MULT_F has fraction part.", CLKFBOUT_PHASE);
//        $finish;
//      end
  ...
  always @(posedge dclk_in or posedge GSR)
    if (GSR == 1) begin
      ...
      if (den_in == 1) begin
        ...
        if (drp_lock == 1) begin
          $display(" Warning : DEN is high at MMCME2_ADV instance %m at time %t. Need wait for DRDY signal before next read/write operation through DRP. ", $time);
        end
        else begin
          drp_lock <= 1;
          daddr_lat <= daddr_in;
        ...
        if (valid_daddr && ( daddr_in == 7'b1110100 || daddr_in == 7'b1001111 ||
          ...
          $display(" Warning : Address DADDR=%b is unsupported at MMCME2_ADV instance %m at time %t.  ",  DADDR, $time);
        end
          ...
          (daddr_in >= 7'b0000110 && daddr_in <= 7'b0010110))) begin
             ...
             if (daddr_in == 7'b0010100) begin
               ...
               if (clkfbtmp_divi > M_MAX || (clkfbtmp_divi < M_MIN))
                  $display(" Input Error : DI at Address DADDR=%b is %h at MMCME2_ADV instance %m at time %t. The sum of DI[11:6] and DI[5:0] is %h and over the range of %f to %f.",  daddr_in, di_in, clkfbtmp_divi, $time, M_MIN, M_MAX);
            end
             ...
             if (daddr_in == 7'b0010110) begin
               ...
               if (clkind_divi > D_MAX || (clkind_divi < 1 && clkind_nocnti == 0))
                  $display(" Input Error : DI at Address DADDR=%b is %h at MMCME2_ADV instance %m at time %t. The sum of DI[11:6] and DI[5:0] is %d and over the range of 1 to %d.",  daddr_in, di_in, clkind_divi, $time, D_MAX);
            end
          end
          else begin
                  $display(" Error : RST is low at MMCME2_ADV instance %m at time %t. RST need to be high when change MMCME2_ADV paramters through DRP. ", $time);
          end
        end //DWE
    end  //DEN
    if ( drp_lock == 1) begin
  ...
  always @(posedge clkinstopped_out or posedge rst_in)
    if (rst_in)
     ...
     if (clkin_hold_f == 1) begin
       ...
       if (clkinsel_in == 1)
        $display(" Error : input CLKIN1 of MMCME2_ADV on instance %m is stopped. Reset is required for MMCME2_ADV whether stopped input clock comes back or not.");
       else
        $display(" Error : input CLKIN2 of MMCME2_ADV on instance %m is stopped. Reset is required for MMCME2_ADV whether stopped input clock comes back or not.");
     end
    end
...
//          val_tmp = period_vco_mf;
  ...
  always @(posedge psclk_in or posedge rst_in)
    if (rst_in) begin
     ...
     if (psen_in) begin
       if (psen_w == 1)
        $display(" Error : PSEN on MMCME2_ADV instance %m is active more than 1 PSCLK period at time %t. PSEN must be active for only one PSCLK period.", $time);

       psen_w <= 1;
       if (ps_lock == 1) 
        $display(" Warning : Please wait for PSDONE signal on MMCME2_ADV instance %m at time %t before adjusting the Phase Shift.", $time);
...
//  integer clkdiv;
  ...
  begin
    ...
    if (clkout_dly_tmp > 63) begin
      $display(" Warning : Attribute %s of MMCME2_ADV on instance %m is set to %f. Required phase shifting can not be reached since it is over the maximum phase shifting ability of MMCME2_ADV", clk_ps_name, clk_ps);
    ...
    if (((clk_ps_rl- clk_ps) > 0.001) || ((clk_ps_rl- clk_ps) < -0.001))
      $display(" Warning : Attribute %s of MMCME2_ADV on instance %m is set to %f. Real phase shifting is %f. Required phase shifting can not be reached.", clk_ps_name, clk_ps, clk_ps_rl);
  ...
  begin
    if (di_in_tmp[12] != 1) begin
      $display(" Error : MMCME2_ADV on instance %m input DI is %h at address DADDR=%b at time %t. The bit 12 need to be set to 1 .", di_in_tmp, daddr_in_tmp, $time); 
    end

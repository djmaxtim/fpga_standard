...
//    reg [15:0]  ot_sf_limit_low_reg = 16'hAE40;
    ...
    reg [7:0] alarm_out;
    reg       ot_out;
    reg [15:0] curr_seq, curr_seq2, curr_seq_m;
...
//  `endif  `ifdef XIL_TIMING
    ...
    initial begin
        ...
        if ((init41h_tmp[15:12]==4'b0011) && (init40h_tmp[8]==1) && (init40h_tmp[4:0] != 5'b00011) && (init40h_tmp[4:0] < 5'b10000))
             $display(" Attribute Syntax warning : The attribute INIT_40 on X_XADC instance %m is set to %x.  Bit[8] of this attribute must be set to 0. Long acquistion mode is only allowed for external channels", INIT_40);

        if ((init41h_tmp[15:12]!=4'b0011) && (init4eh_tmp[10:0]!=11'b0) && (init4eh_tmp[15:12]!=4'b0))
             $display(" Attribute Syntax warning : The attribute INIT_4E on X_XADC instance %m is set to %x.  Bit[15:12] and bit[10:0] of this attribute must be set to 0. Long acquistion mode is only allowed for external channels", INIT_4E);

        if ((init41h_tmp[15:12]==4'b0011) && (init40h_tmp[9]==1) && (init40h_tmp[4:0] != 5'b00011) && (init40h_tmp[4:0] < 5'b10000))
             $display(" Attribute Syntax warning : The attribute INIT_40 on X_XADC instance %m is set to %x.  Bit[9] of this attribute must be set to 0. Event mode timing can only be used with external channels, and only in single channel mode.", INIT_40);

        if ((init41h_tmp[15:12]==4'b0011) && (init40h_tmp[13:12]!=2'b00) && (INIT_48 != 16'h0000) &&  (INIT_49 != 16'h0000))
             $display(" Attribute Syntax warning : INIT_48 and INIT_49 are %x and %x on X_XADC instance %m. Those attributes must be set to 0000h in single channel mode and averaging enabled.", INIT_48, INIT_49);

        if (init42h_tmp[1:0] != 2'b00) 
             $display(" Attribute Syntax Error : The attribute INIT_42 on X_XADC instance %m is set to %x.  Bit[1:0] of this attribute must be set to 0h.", INIT_42);

//          if (init42h_tmp[15:8] < 8'b00000010) begin 
//             $display(" Attribute Syntax Error : The attribute INIT_42 on X_XADC instance %m is set to %x.  Bit[15:8] of this attribute is the ADC Clock divider and must be equal or greater than 2. ", INIT_42);
//           $finish;
//          end
        
        if (INIT_43 != 16'h0) 
             $display(" Warning : The attribute INIT_43 on X_XADC instance %m is set to %x.  This must be set to 0000h.", INIT_43);
             
        if (INIT_44 != 16'h0) 
             $display(" Warning : The attribute INIT_44 on X_XADC instance %m is set to %x. This must be set to  0000h.", INIT_44);

        if (INIT_45 != 16'h0) 
             $display(" Warning : The attribute INIT_45 on X_XADC instance %m is set to %x.  This must be set to 0000h.", INIT_45);

        if (INIT_46 != 16'h0) 
             $display(" Warning : The attribute INIT_46 on X_XADC instance %m is set to %x.  This must be set to  0000h.", INIT_46);

        if (INIT_47 != 16'h0) 
             $display(" Warning : The attribute INIT_47 on X_XADC instance %m is set to %x.  This must be set to 0000h.", INIT_47);
...
// read input file
	...
	if  (fd == 0)
        begin
          $display(" *** Warning: The analog data file %s for X_XADC instance %m was not found. Use the SIM_MONITOR_FILE parameter to specify the analog data file name or use the default name: design.txt.\n", SIM_MONITOR_FILE);
          sim_file_flag = 1;
        end
...
//             if(char_2==`EOFile) 
      ...
      label[42] = label42;
		...
		for (m = 0; m < num_arg; m = m +1) begin
			...
			default : begin
	              $display("analog Data File Error : The channel name %s is invalid in the input file for X_XADC instance %m.", tmp_label);
                                      infile_format;
 			          end
		    endcase
      ...
      chan_valn[31] = column_real[vauxn_idx15];
		...
		if (time_index != -1) begin
			...
			$display("analog Data File Error : Time value %f is invalid in the input file for X_XADC instance %m. Time value should increase.", time_out);
                        infile_format;
		    end
		    ...
		    $display("analog Data File Error : No TIME label is found in the analog data file for X_XADC instance %m.");
                    infile_format;
		    $finish;
		end
...
//		# ((time_out - prev_time_out) * 1000);
    ...
    task infile_format;
    begin
    $display("\n***** X_XADC Simulation analog Data File Format *****\n");
...
// Check if (Vp+Vn)/2 = 0.5 +/- 100 mv,  unipolar only
    ...
    begin
      if (acq_b_u == 0 && rst_in == 0 && ((acq_chan == 3) || (acq_chan >= 16 && acq_chan <= 31))) begin  
            ...
            $display("Input File Warning: The N input for external channel %x must be smaller than P input when in unipolar mode (P=%0.2f N=%0.2f) for X_XADC instance %m at %.3f ns.", acq_chan, chan_val_p_tmp, chan_val_n_tmp, $time/1000.0);
            if ( chan_val_n_tmp > 0.5 || chan_val_n_tmp < 0.0)
            $display("Input File Warning: The range of N input for external channel %x should be between 0V to 0.5V when in unipolar mode (N=%0.2f) for X_XADC instance %m at %.3f ns.", acq_chan, chan_val_n_tmp, $time/1000.0);
      end
      if ((seq1_0[3:2] == 2'b01 || seq1_0[3:2] == 2'b10) && acq_b_u == 0 && rst_in == 0 && ((acq_chan2 == 3) || (acq_chan2 >= 16 && acq_chan2 <= 31))) begin  
            ...
            if ( chan_val_n_tmp > chan_val_p_tmp)
            $display("Input File Warning: The N input for external channel %x must be smaller than P input when in unipolar mode (P=%0.2f N=%0.2f) for X_XADC instance %m at %.3f ns.", acq_chan2, chan_val_p_tmp, chan_val_n_tmp, $time/1000.0);
            if ( chan_val_n_tmp > 0.5 || chan_val_n_tmp < 0.0)
            $display("Input File Warning: The range of N input for external channel %x should be between 0V to 0.5V when in unipolar mode (N=%0.2f) for X_XADC instance %m at %.3f ns.", acq_chan2, chan_val_n_tmp, $time/1000.0);
      end
    end
     ...
     if (gsr_in == 1) begin
        ...
        if (den_in == 1'b1) begin
          if (drdy_out_tmp1 == 1'b0) begin
             ...
             $display("Warning : input pin DEN on X_XADC instance %m at time %.3f ns can not continue set to high. Need wait DRDY high and then set DEN high again.", $time/1000.0);  
          end
        end
        else
           drdy_out_tmp1 <= 1'b0;
        ...
        if (den_in == 1 && (daddr_in >7'h5F || (daddr_in >= 7'h2F && daddr_in < 7'h3F)))
        $display("Invalid Input Warning : The DADDR %x to X_XADC instance %m at time %.3f ns is accessing an undefined location. The data in this location is invalid.", daddr_in, $time/1000.0);
...
//                    ot_limit_reg <= 16'hCA00;
            ...
            if ( daddr_in == 7'h42 && (di_in[2:0] !=3'b000)) 
                $display(" Invalid Input Error : The DI bit[2:0] %x at DADDR %x on X_XADC instance %m at %.3f ns is invalid. These must be set to 000.", di_in[2:0], daddr_in, $time/1000.0);

          if ( daddr_in >= 7'h43 && daddr_in <= 7'h47 && (di_in[15:0] != 16'h0000)) 
		$display(" Invalid Input Error : The DI value %x at DADDR %x of X_XADC instance %m at %.3f ns is invalid. These must be set to 0000h.", di_in, daddr_in, $time/1000.0);

          if ((daddr_in == 7'h40) && ( di_in[4:0] == 5'b00111
                         || (di_in[4:0] >= 5'b01001 && di_in[4:0] <= 5'b01100))) 
		$display("Invalid Input Warning : The DI bit4:0] at address DADDR %x to X_XADC instance %m at %.3f ns is %h, which is invalid analog channel.", daddr_in, $time/1000.0, di_in[4:0]);

          if (daddr_in == 7'h40) begin
            if ((cfg_reg1[15:12]==4'b0011) && (di_in[8]==1) && (di_in[4:0] != 5'b00011) && (di_in[4:0] < 5'b10000))
             $display(" Invalid Input warning : The DI value is %x at DADDR %x on X_XADC instance %m at %.3f ns.  Bit[8] of DI must be set to 0. Long acquistion mode is only allowed for external channels", di_in, daddr_in, $time/1000.0);

          if ((cfg_reg1[15:12]==4'b0011) && (di_in[9]==1) && (di_in[4:0] != 5'b00011) && (di_in[4:0] < 5'b10000))
             $display(" Invalid Input warning : The DI value is %x at DADDR %x on X_XADC instance %m at %.3f ns.  Bit[9] of DI must be set to 0. Event mode timing can only be used with external channels", di_in, daddr_in, $time/1000.0);

          if ((cfg_reg1[15:12]==4'b0011) && (di_in[13:12]!=2'b00) && (seq_chan_reg1 != 16'h0000) &&  (seq_chan_reg2 != 16'h0000))
             $display(" Invalid Input warning : The  Control Regiter 48h and 49h are %x and %x on X_XADC instance %m at %.3f ns. Those registers should be set to 0000h in single channel mode and averaging enabled.", seq_chan_reg1, seq_chan_reg2, $time/1000.0);

         end

         if (daddr_in == 7'h41 && en_data_flag == 1) begin
           if ((di_in[15:12]==4'b0011) && (cfg_reg0[8]==1) && (cfg_reg0[4:0] != 5'b00011) && (cfg_reg0[4:0] < 5'b10000))
             $display(" Invalid Input warning : The  Control Regiter 40h value is %x on X_XADC instance %m at %.3f ns.  Bit[8] of Control Regiter 40h  must be set to 0. Long acquistion mode is only allowed for external channels", cfg_reg0, $time/1000.0);

           if ((di_in[15:12]==4'b0011) && (cfg_reg0[9]==1) && (cfg_reg0[4:0] != 5'b00011) && (cfg_reg0[4:0] < 5'b10000))
             $display(" Invalid Input warning : The  Control Regiter 40h value is %x on X_XADC instance %m at %.3f ns.  Bit[9] of Control Regiter 40h  must be set to 0. Event mode timing can only be used with external channels", cfg_reg0, $time/1000.0);

           if ((di_in[15:12]!=4'b0011) && (seq_acq_reg1[10:0]!=11'b0) && (seq_acq_reg1[15:12]!=4'b0))
             $display(" Invalid Input warning : The Control Regiter 4Eh value is %x on X_XADC instance %m at %.3f ns.  Bit[15:12] and bit[10:0] of this register must be set to 0. Long acquistion mode is only allowed for external channels", seq_acq_reg1, $time/1000.0);
...
// latch configuration registers
  ...
  always @( seq1_0 or adc_s1_flag or curr_seq or curr_seq2 or cfg_reg0_adc or rst_in) begin
   ...
   always @(posedge conv_start or  rst_in ) 
	if (rst_in == 1) begin
       ...
       curr_chan2 <= 5'b0;
		...
		$display("Invalid Input Warning : The analog channel %x to X_XADC instance %m at %.3f ns is invalid.", acq_chan, $time/1000.0);
...
// sequence control
    ...
    always @( seq_count2  or negedge seq_en_dly) begin
        ...
        if (seq_curr_i2 >= 0 && seq_curr_i2 <= 15) begin
           curr_seq2 [2:0] =  seq_curr_i2[2:0];
           curr_seq2 [4:3] = 2'b01;
           curr_seq2 [8] = seq_acq_reg1[seq_curr_i2];
           curr_seq2 [9] = 0;
           curr_seq2 [10] = seq_du_reg1[seq_curr_i2];

           if ( seq1_0[3:2] == 2'b10)
              curr_seq2 [13:12] = 2'b01;
           else
              curr_seq2 [13:12] = 2'b00;
...
//	if (rst_in == 1) begin
	if (rst_in == 1 || rst_lock == 1) begin
		...
		if (next_state == S3_ST)
			...
			$display ("Warning: Acquisition time is not long enough for X_XADC instance %m at time %t.", $time);

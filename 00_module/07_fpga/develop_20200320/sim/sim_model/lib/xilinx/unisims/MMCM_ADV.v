// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/blanc/MMCM_ADV.v,v 1.55 2011/05/05 19:00:53 yanx Exp $
///////////////////////////////////////////////////////////////////////////////
...
module MMCM_ADV (
  ...
  localparam real CLKIN_FREQ_MAX = 800.0;
  ...
  always @(posedge clkinstopped_out or posedge rst_in) 
    if (rst_in)
     ...
     if (clkin_hold_f == 1) begin
       ...
       if (clkinsel_in == 1)
        $display(" Error : input CLKIN1 of MMCM_ADV on instance %m is stopped. Reset is required for MMCM_ADV whether stopped input clock comes back or not.");
       else
        $display(" Error : input CLKIN2 of MMCM_ADV on instance %m is stopped. Reset is required for MMCM_ADV whether stopped input clock comes back or not.");
     end
    end
...
//        val_tmp = period_vco * mf_product ;
  ...
  always @(posedge psclk_in or posedge rst_in)
    if (rst_in) begin
     ...
     if (psen_in) begin
       if (psen_w == 1)
        $display(" Error : PSEN on MMCM_ADV instance %m is active more than 1 PSCLK period at time %t. PSEN must be active for only one PSCLK period.", $time);

       psen_w <= 1;
       if (ps_lock == 1) 
        $display(" Warning : Please wait for PSDONE signal on MMCM_ADV instance %m at time %t before adjusting the Phase Shift.", $time);

// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/verunilibs/data/fuji/PLLE2_ADV.v,v 1.16 2011/03/10 20:37:09 yanx Exp $
///////////////////////////////////////////////////////////////////////////////
...
module PLLE2_ADV (
  ...
  always @(pll_locked_tmp2)
      pll_locked_tmp2_dly = #100 pll_locked_tmp2;
  always @(drdy_out)
             ...
             if (daddr_in == 7'b0010100) begin
               ...
               if (SIM_DEVICE == "VIRTEX6") begin
                 if (clkfbtmp_divi > 64 || (clkfbtmp_divi < 5))
                  $display(" Input Error : DI at Address DADDR=%b is %h at PLLE2_ADV instance %m at time %t. The sum of DI[11:6] and DI[5:0] is %b and over the range of %d to %d.",  daddr_in, di_in, clkfbtmp_divi, $time, 5, 64);
               end
               else begin
                 if (clkfbtmp_divi > M_MAX || (clkfbtmp_divi < M_MIN))
                  $display(" Input Error : DI at Address DADDR=%b is %h at PLLE2_ADV instance %m at time %t. The sum of DI[11:6] and DI[5:0] is %d and over the range of %d to %d.",  daddr_in, di_in, clkfbtmp_divi, $time, M_MIN, M_MAX);
               end
            end
  ...
  always @(posedge clkinstopped_out or posedge rst_in)
    if (rst_in)
     ...
     if (clkin_hold_f == 1) begin
       ...
       if (clkinsel_in == 1)
        $display(" Error : input CLKIN1 of PLLE2_ADV on instance %m is stopped. Reset is required for PLLE2_ADV whether stopped input clock comes back or not.");
       else
        $display(" Error : input CLKIN2 of PLLE2_ADV on instance %m is stopped. Reset is required for PLLE2_ADV whether stopped input clock comes back or not.");
     end
    end

// $Header: /devl/xcs/repo/env/Databases/CAEInterfaces/versclibs/data/stan/X_IODRP2_MCB.v,v 1.18 2011/01/06 21:28:39 robh Exp $
///////////////////////////////////////////////////////
...
// /__/   /\       Filename    : X_IODRP2_MCB.v
// \  \  /  \ 
//  \__\/\__ \                      
//                                   
//  Revision Date:  Comment
//     09/30/2008:  Initial version. From IODELAY2/IODRP2
//     11/05/2008   I/O, structure change
//                  Correct BKST functionality
//     11/19/2008   Change SIM_TAP_DELAY to SIM_TAPDELAY_VALUE
//     11/21/2008   Change AUX_SDO_OUTDELAY to AUXSDO_OUTDELAY
//     01/29/2009   IR504942, 504805, 504692 sync to data
//                  IR506027 DQSOUT's
//     02/04/2009   CR506027 IODRP state machine, sync_to_data off
...
//-----------------------------------------------------------------------------
  ...
  localparam MODULE_NAME = "X_IODRP2_MCB";
  localparam IDLE = 0,
             ADDR_ACTIVE = 1,
             DATA_ACTIVE = 2;
  ...
  reg [7:0] idelay_val_pe_m_reg1 = 0;
  reg [7:0] idelay_val_pe_s_reg1 = 0;
  reg [7:0] idelay_val_ne_reg;
  reg [7:0] idelay_val_ne_m_reg = 0;
  reg [7:0] idelay_val_ne_s_reg = 0;
  reg [7:0] idelay_val_ne_m_reg1 = 0;
  reg [7:0] idelay_val_ne_s_reg1 = 0;
...
//-----------------------------------------------------------------------------
   ...
   always @(posedge delay_val_pe_clk or posedge rst_sig or posedge busy_out_int) begin
      if ((rst_sig == 1'b1) || 
          (((busy_out_int == 1'b1)&&(busy_out_pe_one_shot == 1'b0))||(cal_count < 5'b10011)))
         begin
   ...
   always @(posedge delay_val_ne_clk or posedge rst_sig or posedge busy_out_int) begin
      if ((rst_sig == 1'b1) || 
         (((busy_out_int == 1'b1)&&(busy_out_ne_one_shot == 1'b0))||(cal_count < 5'b10011)))
         begin
   ...
   always @(posedge delay_val_pe_clk or posedge rst_sig) begin
      if ((rst_sig == 1'b1) || (busy_out_dly == 1'b0)) begin
         idelay_val_pe_reg <=
              (SERDES_MODE_BINARY == SLAVE) ?  idelay_val_pe_s_reg : idelay_val_pe_m_reg;
         end
      else begin
         idelay_val_pe_reg <=
              (SERDES_MODE_BINARY == SLAVE) ?  idelay_val_pe_s_reg1 : idelay_val_pe_m_reg1;
         idelay_val_pe_s_reg1 <= idelay_val_pe_s_reg;  
         idelay_val_pe_m_reg1 <= idelay_val_pe_m_reg;
      end
   end

   always @(posedge delay_val_ne_clk or posedge rst_sig) begin
      if ((rst_sig == 1'b1) || (busy_out_dly == 1'b0)) begin
         idelay_val_ne_reg <=
         ...
         idelay_val_ne_reg <=
              (SERDES_MODE_BINARY == SLAVE) ?  idelay_val_ne_s_reg1 : idelay_val_ne_m_reg1;
         idelay_val_ne_s_reg1 <= idelay_val_ne_s_reg;  
         idelay_val_ne_m_reg1 <= idelay_val_ne_m_reg;
      end
   end
...
//-----------------------------------------------------------------------------
   ...
   always @(posedge CLK_INDELAY or posedge GSR_INDELAY) begin
      if (GSR_INDELAY) begin
         ...
         cal_count <= 5'b10011;
         busy_out_int <= 1'b1; // inc (busy high 1 clock)
         end
...
//-----------------------------------------------------------------------------
  ...
  buf B_AUXSDO (AUXSDO, AUXSDO_OUTDELAY);
  buf B_DATAOUT (DATAOUT, DATAOUT_OUTDELAY);
  buf B_DATAOUT2 (DATAOUT2, DATAOUT2_OUTDELAY);
  buf B_DOUT (DOUT, DOUT_OUTDELAY);
  buf B_DQSOUTN (DQSOUTN, DQSOUTN_OUTDELAY);
  buf B_DQSOUTP (DQSOUTP, DQSOUTP_OUTDELAY);
  buf B_SDO (SDO, SDO_OUTDELAY);
  buf B_TOUT (TOUT, TOUT_OUTDELAY);

  buf B_ADD (ADD_IN, ADD);
  buf B_AUXADDR0 (AUXADDR_IN[0], AUXADDR[0]);
  buf B_AUXADDR1 (AUXADDR_IN[1], AUXADDR[1]);
  buf B_AUXADDR2 (AUXADDR_IN[2], AUXADDR[2]);
  buf B_AUXADDR3 (AUXADDR_IN[3], AUXADDR[3]);
  buf B_AUXADDR4 (AUXADDR_IN[4], AUXADDR[4]);
  buf B_AUXSDOIN (AUXSDOIN_IN, AUXSDOIN);
  buf B_BKST (BKST_IN, BKST);
  buf B_CS (CS_IN, CS);
  buf B_IDATAIN (IDATAIN_IN, IDATAIN);
  buf B_IOCLK0 (IOCLK0_IN, IOCLK0);
  buf B_IOCLK1 (IOCLK1_IN, IOCLK1);
  buf B_MEMUPDATE (MEMUPDATE_IN, MEMUPDATE);
  buf B_ODATAIN (ODATAIN_IN, ODATAIN);
  buf B_SDI (SDI_IN, SDI);
  buf B_T (T_IN, T);


  specify
    $setuphold (posedge CLK, negedge ADD, 0:0:0, 0:0:0, notifier,,, CLK_IN, ADD_IN);
    $setuphold (posedge CLK, negedge BKST, 0:0:0, 0:0:0, notifier,,, CLK_IN, BKST_IN);
    $setuphold (posedge CLK, negedge CS, 0:0:0, 0:0:0, notifier,,, CLK_IN, CS_IN);
    $setuphold (posedge CLK, negedge SDI, 0:0:0, 0:0:0, notifier,,, CLK_IN, SDI_IN);
    $setuphold (posedge CLK, posedge ADD, 0:0:0, 0:0:0, notifier,,, CLK_IN, ADD_IN);
    $setuphold (posedge CLK, posedge BKST, 0:0:0, 0:0:0, notifier,,, CLK_IN, BKST_IN);
    $setuphold (posedge CLK, posedge CS, 0:0:0, 0:0:0, notifier,,, CLK_IN, CS_IN);
    $setuphold (posedge CLK, posedge SDI, 0:0:0, 0:0:0, notifier,,, CLK_IN, SDI_IN);
    ( CLK => SDO) = (0:0:0, 0:0:0);
    ( IDATAIN => DATAOUT) = (0:0:0, 0:0:0);
    ( IDATAIN => DATAOUT2) = (0:0:0, 0:0:0);
    ( ODATAIN => DOUT) = (0:0:0, 0:0:0);
    ( T => TOUT) = (0:0:0, 0:0:0);

    specparam PATHPULSE$ = 0;
  endspecify
endmodule // X_IODRP2_MCB

...
// /___/   /\     Filename : X_SIM_CONFIG_S6.v
// \   \  /  \    Timestamp : 
//  \___\/\___\
//
// Revision:
...
//    04/22/11 - Move icap_on check before DEVICE_ID check (CR606812)
...
//  assign glbl.GWE = GWE;
  ...
  initial begin
    ...
    if (DEVICE_ID == 32'h0 && icap_on == 0) begin
      $display("Attribute Error : The attribute DEVICE_ID on  X_SIM_CONFIG_S6 instance %m is not set.");
     end
...
//            @(posedge cclk_in);
    ...
    if (prog_b_t == 0)
       ...
       if (prog_b_t == 1)  begin
          ...
          if (m_in !== 2'b10) begin
             mode_sample_flag <=  0;
             if ( icap_on == 0)
               $display("Error: input M is %h. Only Slave SelectMAP mode M=10 supported on X_SIM_CONFIG_S6 instance %m.", m_in);
          end
          else
  ...
  always @(posedge init_b_t )
       if (prog_b_t != 1)  begin
            if ($time != 0 && icap_on == 0)
       $display("Error: PROGB is not high when INITB goes high on X_SIM_CONFIG_S6 instance %m at time %t.", $time);
    end

  always @(m_in)
    if (mode_sample_flag == 1 && persist_en == 1 && icap_on == 0)
       $display("Error : Mode pine M[2:0] changed after rising edge of INITB on X_SIM_CONFIG_S6 instance %m at time %t.", $time);
  ...
  always @(posedge prog_b_in or negedge prog_b_in)
    if (prog_b_in ==0) 
       ...
       if (prog_pulse_low < cfg_Tprog  && icap_on == 0)
        $display("Error: Low time of PROGB is less than required minimum Tprogram time %d on X_SIM_CONFIG_S6 instance %m at time %t.", cfg_Tprog, $time);
    end
               ...
               6'b001110 : 
                          if (conti_data_cnt==5'b00001) begin
                             ...
                             if (idcode_tmp[27:0] != DEVICE_ID[27:0]) begin
                                id_error_flag <= 1;
                                if (icap_on == 0)
                                $display("Error :  written value to IDCODE register is %h which does not match DEVICE ID %h on X_SIM_CONFIG_S6 instance %m at time %t.", idcode_tmp, DEVICE_ID, $time);
                                else
                                $display("Error :  written value to IDCODE register is %h which does not match DEVICE ID %h on X_ICAP_SPARTAN6 instance %m at time %t.", idcode_tmp, DEVICE_ID, $time);
                             end 
                             else
                             id_error_flag <= 0;
                          end
      ...
      if (rst_intl==0 || abort_flag_rst==1 || csi_b_in == 1)
        ...
        if (abort_dis == 0 && csi_b_in == 0) begin
             if ($time != 0) begin
               abort_flag_wr <= 1;
               if (icap_on == 0)
               $display(" Warning : RDWRB changes when CS_B low, which causes Configuration abort on X_SIM_CONFIG_S6 instance %m at time %t.", $time);
             end
        end
        else
           abort_flag_wr <= 0;
...
//            abort_status <= {cfgerr_b_flag, bus_sync_flag, 1'b0, 1'b1, 12'b1111};
            abort_status <= {12'hFFF, 1'b1, 1'b0, bus_sync_flag, cfgerr_b_flag};
         else if (abort_cnt== 1)
            abort_status <= {12'hFFF, 1'b0, 1'b0,  1'b1, cfgerr_b_flag};
         else if (abort_cnt== 2)
            abort_status <= {12'hFFF, 1'b0,  1'b0,  1'b0, cfgerr_b_flag};
         else if (abort_cnt== 3)
            abort_status <= {12'hFFF, 1'b1, 1'b0,  1'b0, cfgerr_b_flag};

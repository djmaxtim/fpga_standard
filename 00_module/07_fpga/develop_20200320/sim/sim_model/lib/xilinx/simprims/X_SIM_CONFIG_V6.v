...
//    02/18/11 - Change cfg_Tprog to 250 ns (CR593126)
...
module X_SIM_CONFIG_V6 ( BUSY,
  ...
  localparam cfg_Tprog = 250000;   // min PROG must be low, 250 ns
...
//  tri0 GSR, GTS, GWE;
  ...
  reg  shutdown_set = 0;
  reg  desynch_set = 0;
  reg  [2:0] done_cycle_reg = 3'b011;
  reg  [2:0] gts_cycle_reg = 3'b101;
  reg  [2:0] gwe_cycle_reg = 3'b100;
  reg  init_pin;
  reg  init_rst = 0;
...
//  reg rst_en = 1;
  ...
  initial begin
    ...
    if (DEVICE_ID == 32'h0 && icap_on == 0) begin
      $display("Attribute Error : The attribute DEVICE_ID on  X_SIM_CONFIG_V6 instance %m is not set.");
     end
...
//   assign d_out[7:0] = (abort_out_en ) ? abort_status : outbus[7:0];
    ...
    if (prog_b_t == 0)
       ...
       if (prog_b_t == 1)  begin
          ...
          if (m_in !== 3'b110) begin
             mode_sample_flag <=  0;
             if ( icap_on == 0) 
               $display("Error: input M is %h. Only Slave SelectMAP mode M=110 supported on X_SIM_CONFIG_V6 instance %m.", m_in);
          end
          else
  ...
  always @(posedge init_b_t ) 
       if (prog_b_t != 1)  begin
            if ($time != 0 && icap_on == 0)
       $display("Error: PROGB is not high when INITB goes high on X_SIM_CONFIG_V6 instance %m at time %t.", $time);
    end

  always @(m_in)
    if (mode_sample_flag == 1 && persist_en == 1 && icap_on == 0)
       $display("Error : Mode pine M[2:0] changed after rising edge of INITB on X_SIM_CONFIG_V6 instance %m at time %t.", $time);
  ...
  always @(posedge prog_b_in or negedge prog_b_in)
    if (prog_b_in == 0) 
       ...
       if (prog_pulse_low < cfg_Tprog  && icap_on == 0)
        $display("Error: Low time of PROGB is less than required minimum Tprogram time %d on X_SIM_CONFIG_V6 instance %m at time %t.", cfg_Tprog, $time);
    end
           ...
           if (bus_en == 1 && rdwr_b_in == 0)  begin
                 ...
                 if (buswid_flag_init == 0) begin
                     ...
                     if (tmp_byte == 8'h11) begin
                         ...
                         buswid_flag_init <= 0;
                         if (icap_on == 0) 
                         $display("Error : BUS Width Auto Dection did not find 0x11 or 0x22 or 0x44 on D[7:0] followed 0xBB on X_SIM_CONFIG_V6 instance %m at time %t.", $time);
                         else 
                         $display("Error : BUS Width Auto Dection did not find 0x11 or 0x22 or 0x44 on dix[7:0] followed 0xBB on X_ICAP_VIRTEX6 instance %m at time %t.", $time);
...
//    always @(posedge cclk_in or posedge desync_flag or posedge csi_b_in)
    always @(posedge cclk_in or posedge desync_flag or negedge csi_b_in)
    ...
    always @(negedge cclk_in or negedge rst_intl)
      if (rst_intl == 0) begin
        ...
        if (new_data_in_flag == 1 && wr_flag == 1 && csi_b_ins == 0) begin
           if (conti_data_flag == 1 ) begin
               ...
               5'b01100 : begin
                          ...
                          if (pack_in_reg[27:0] != DEVICE_ID[27:0]) begin
                             id_error_flag <= 1;
                             if (icap_on == 0)
                             $display("Error : written value to IDCODE register is %h which does not match DEVICE ID %h on X_SIM_CONFIG_V6 instance %m at time %t.", pack_in_reg, DEVICE_ID, $time);
                             else
                             $display("Error : written value to IDCODE register is %h which does not match DEVICE ID %h on X_ICAP_VIRTEX6 instance %m at time %t.", pack_in_reg, DEVICE_ID, $time);
                          end 
                          else
                             id_error_flag <= 0;
                          end
...
//   always @(posedge startup_set or posedge desynch_set or negedge  rw_en )
   always @(posedge startup_set or posedge desynch_set )
    if (rw_en == 0)
      ...
      if (rst_intl == 0 || abort_flag_rst == 1 || csi_b_in == 1) begin
        ...
        if (  abort_dis == 0 && csi_b_in == 0) begin
             if ((rdwr_b_in1 != rdwr_b_in) && checka_en != 0) begin
               abort_flag <= 1;
               if (icap_on == 0)
               $display(" Warning : RDWRB changes when CSB low, which causes Configuration abort on X_SIM_CONFIG_V6 instance %m at time %t.", $time);
             end
        end
        else
           abort_flag <= 0;

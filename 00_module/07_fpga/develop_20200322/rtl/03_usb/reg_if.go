...
module REG_IF (
    ...
    //PCIE RX interface
    input                               REG_WR_REQ                              ,//(i)  [  1]  PCIE register write request
    input                               REG_RD_REQ                              ,//(i)  [  1]  PCIE register read request
    input       [  31:0]                REG_WR_DATA                             ,//(i)  [ 32]  PCIE register write data
    input       [  31:0]                REG_OP_ADDR                             ,//(i)  [ 64]  PCIE register operation address
    output      [  31:0]                REG_RD_DATA                             ,//(o)  [ 32]  PCIE register read data
    output                              REG_WR_ACK                              ,//(o)  [  1]  PCIE register write acknowledge
    output                              REG_RD_ACK                              ,//(o)  [  1]  PCIE register read acknowledge
    //REG CTRL interface
...
// =============================================================================
    ...
    reg                                 r_REG_WR_ACK                           ;//(r)  [  1]  PCIE register write acknowledge
    reg                                 r_REG_RD_ACK                           ;//(r)  [  1]  PCIE register read acknowledge
    reg         [  31:0]                r_REG_RD_DATA                          ;//(r)  [ 32]  PCIE register read data
    ...
    always @ (posedge CLK or posedge RST) begin
        if (RST) begin
            ...
            if (REG0_RD_ACK) begin
                r_REG_RD_DATA           <= REG0_RD_DATA ;
            end else if (REG1_RD_ACK) begin
                r_REG_RD_DATA           <= REG1_RD_DATA ;
            end else if (REG2_RD_ACK) begin
                r_REG_RD_DATA           <= REG2_RD_DATA ;
            end else if (REG3_RD_ACK) begin
                r_REG_RD_DATA           <= REG3_RD_DATA ;
            end else if (REG4_RD_ACK) begin
                r_REG_RD_DATA           <= REG4_RD_DATA ;
            end else if (REG5_RD_ACK) begin
                r_REG_RD_DATA           <= REG5_RD_DATA ;
            end else if (REG6_RD_ACK) begin
                r_REG_RD_DATA           <= REG6_RD_DATA ;
            end else if (REG7_RD_ACK) begin
                r_REG_RD_DATA           <= REG7_RD_DATA ;
            end
        end
    end

...
//  tri  done_o0, done_o1, done_o2, done_o3;
...
//  reg [3:0] bout_en = 4'b0111;
  ...
  initial begin
    ...
    if (DEVICE_ID == 32'h0 && icap_on == 0) begin
      $display("Attribute Error : The attribute DEVICE_ID on  X_SIM_CONFIGE2 instance %m is not set.");
     end
...
//   always @(csi_b_in or abort_flag[ib]) 
//        if (csi_b_in == 1) 
//           busy_o = 1'b1;
//        else
//          if (abort_flag[ib] == 1)
...
//          end
    ...
    if (prog_b_t == 0)
       ...
       if (prog_b_t == 1)  begin
          ...
          if (m_in !== 3'b110) begin
             mode_sample_flag <=  0;
             if ( icap_on == 0) 
               $display("Error: input M is %h. Only Slave SelectMAP mode M=110 supported on X_SIM_CONFIGE2 instance %m.", m_in);
          end
          else
  ...
  always @(posedge init_b_t ) 
       if (prog_b_t != 1)  begin
            if ($time != 0 && icap_on == 0)
       $display("Error: PROGB is not high when INITB goes high on X_SIM_CONFIGE2 instance %m at time %t.", $time);
    end

  always @(m_in)
    if (mode_sample_flag == 1 && persist_en[0] == 1 && icap_on == 0)
       $display("Error : Mode pine M[2:0] changed after rising edge of INITB on X_SIM_CONFIGE2 instance %m at time %t.", $time);
  ...
  always @(posedge prog_b_in or negedge prog_b_in)
    if (prog_b_in == 0) 
       ...
       if (prog_pulse_low < cfg_Tprog  && icap_on == 0)
        $display("Error: Low time of PROGB is less than required minimum Tprogram time %d on X_SIM_CONFIGE2 instance %m at time %t.", cfg_Tprog, $time);
    end
                 ...
                 tmp_byte = bit_revers8(d_in[7:0]);
                  if (buswid_flag_init[ib] == 0) begin
                     ...
                     if (tmp_byte == 8'h11) begin
                         ...
                         if (icap_on == 0) 
                         $display("Error : BUS Width Auto Dection did not find 0x11 or 0x22 or 0x44 on D[7:0] followed 0xBB on X_SIM_CONFIGE2 instance %m at time %t.", $time);
                         else 
                         $display("Error : BUS Width Auto Dection did not find 0x11 or 0x22 or 0x44 on dix[7:0] followed 0xBB on X_ICAPE2 instance %m at time %t.", $time);
...
//    assign buswidth[ib] = buswidth_ibtmp;
...
//                   pack_in_reg_tmp0 = pack_in_reg_tmps0;
...
//               pack_in_reg_tmp = pack_in_reg[ib];
               ...
               5'b01100 : begin
                          ...
                          if (pack_in_reg_tmp[27:0] != DEVICE_ID[27:0]) begin
                             id_error_flag[ib] <= 1;
                             if (icap_on == 0)
                             $display("Error : written value to IDCODE register is %h which does not match DEVICE ID %h on X_SIM_CONFIGE2 instance %m at time %t.", pack_in_reg[ib], DEVICE_ID, $time);
                             else
                             $display("Error : written value to IDCODE register is %h which does not match DEVICE ID %h on X_ICAPE2 instance %m at time %t.", pack_in_reg[ib], DEVICE_ID, $time);
                          end 
                          else
                             id_error_flag[ib] <= 0;
                          end
...
//                           csbo_cnt[ib] = pack_in_reg_tmp[10:0];
//                           csbo_flag[ib] <= 1;
                           conti_data_flag[ib] = 0;
...
//                           bout_cnt[ib] <= {11'b0, pack_in_reg_tmp[10:0]};
//                           bout_flag[ib] <= 1;
                           bout_flags[ib] <= 1;
                           conti_data_flag[ib] = 0;
                           reg_addr[ib] <= pack_in_reg_tmp[17:13];
...
//        crc_ck[ib] <= 0;
   ...
   always @(posedge cclk_in or negedge rst_intl)
    if (rst_intl == 0) begin
        ...
        if (cmd_rd_flag[ib] == 1 && rdwr_b_in == 1) begin
               case (rd_reg_addr[ib])
   ...
   always @(posedge startup_set[0] or posedge desynch_set[0]  )
    if (rw_en[0] == 0)
       startup_set_pulse0 <= 2'b0;
   else
     begin
   ...
   always @(posedge startup_set[1] or posedge desynch_set[1] )
    if (rw_en[1] == 0)
       startup_set_pulse1 <= 2'b0;
    else
     begin
   ...
   always @(posedge startup_set[2] or posedge desynch_set[2] )
    if (rw_en[2] == 0)
       startup_set_pulse2 <= 2'b0;
    else 
     begin
   ...
   always @(posedge startup_set[3] or posedge desynch_set[3])
    if (rw_en[3] == 0)
        startup_set_pulse3 <= 2'b0;
    else 
     begin
      ...
      if (rst_intl == 0 || abort_flag_rst == 1 || csi_b_in == 1) begin
        ...
        if (  abort_dis_bi == 0 && csi_b_in == 0) begin
             if ((rdwr_b_in1 != rdwr_b_in) && checka_en != 0) begin
               abort_flag[ib] <= 1;
               if (icap_on == 0)
               $display(" Warning : RDWRB changes when CSB low, which causes Configuration abort on X_SIM_CONFIGE2 instance %m at time %t.", $time);
             end
        end
        else
           abort_flag[ib] <= 0;

        ...
        if (PCIE_RST) begin
            ...
            if (r_DEK_TX_ACK_DFF1) begin
                ...
                r_DEK_TX_DATA[31:00]        <= {
                                               ...
                                               {2'b0}                   , // |      | --packet head R
                                               {3'b0,7'b0000001}          // |      | --packet head payload length 16 byte = 4DW (Number in DW)
                                               };
                ...
                if (r_WR64_FLAG) begin
                    r_DEK_TX_MASK           <= 2'b11                    ;
                    r_DEK_TX_DATA[63:32]    <= s_DMA_DATA1_SWAP         ; // |Data 3|
                    r_DEK_TX_DATA[31:00]    <= s_DMA_DATA0_SWAP         ; // |Data 2|
                end else begin
                    r_DEK_TX_MASK           <= 2'b01                    ;
                    r_DEK_TX_DATA[63:32]    <= 32'd0                    ; // |Unused|
                    r_DEK_TX_DATA[31:00]    <= s_DMA_DATA1_SWAP         ; // |Data 3|
                end

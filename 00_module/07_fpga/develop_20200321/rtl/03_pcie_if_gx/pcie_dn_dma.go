...
module PCIE_DN_DMA # (
    ...
    input                               DN_FBUF_RD_TRIG                         ,//(o)  [  1]
    input       [  63:0]                DN_FBUF_RD_ADDR                         ,//(o)  [  1]
    input       [  23:0]                DN_FBUF_RD_SIZE                         ,//(o)  [  1]
...
// =============================================================================
    ...
    localparam                          p_IDLE          = 6'b000001            ;//(p)  [  7]
    localparam                          p_LBUF_CHK      = 6'b000010            ;//(p)  [  7]
    localparam                          p_TX_REQ        = 6'b000100            ;//(p)  [  7]
    localparam                          p_TX_DATA       = 6'b001000            ;//(p)  [  7]
    localparam                          p_RX_CHK        = 6'b010000            ;//(p)  [  7]
    localparam                          p_EBUF_REQ      = 6'b100000            ;//(p)  [  7]
...
// =============================================================================
    ...
    wire                                s_ST_TX_DATA                            ;//(s)  [  1]
    wire                                s_ST_RX_CHK                             ;//(s)  [  1]
    wire                                s_ST_EBUF_REQ                           ;//(s)  [  1]

    reg         [   7:0]                r_TX_CNT                                ;//(r)  [  8]
    reg         [   7:0]                r_RX_CNT                                ;//(r)  [  8]
    wire                                s_CPLD_DPK_END                          ;//(s)  [  1]
    ...
    reg                                 r_DMA_H1                                ;//(r)  [  1]
    reg                                 r_DMA_TX                                ;//(r)  [  1]
    reg         [  15:0]                r_DMA_DCNT                              ;//(r)  [ 16]
    reg         [   9:0]                r_DMA_PSIZE                              ;//(r)  [ 16]
    reg                                 r_RD64_FLAG                             ;//(r)  [  1]
...
/+============================================================================*/
    ...
    assign  s_ST_TX_DATA                = r_FSM[3] ;
    assign  s_ST_RX_CHK                 = r_FSM[4] ;
    assign  s_ST_EBUF_REQ               = r_FSM[5] ;
...
/+========================================================================*/
    ...
    always @(posedge PCIE_CLK or posedge PCIE_RST) begin
        if (PCIE_RST) begin
            ...
            if (s_ST_IDLE) begin
                r_TX_CNT                <= 8'h0 ;
            end else if (s_ST_TX_DATA & r_DPK_TX_SOP) begin
    ...
    always @(posedge PCIE_CLK or posedge PCIE_RST) begin
        if (PCIE_RST) begin
            ...
            if (s_ST_IDLE) begin
                r_RX_CNT                <= 8'h0 ;
            end else if ((s_ST_TX_DATA | s_ST_RX_CHK) & r_DN_CX_EOP) begin
                r_RX_CNT                <= r_RX_CNT + 1'b1 ;
            end
        end
    end

    assign s_CPLD_DPK_END              = s_ST_RX_CHK & (r_RX_CNT == r_TX_CNT) ;
...
/+========================================================================*/
    ...
    always @(posedge PCIE_CLK or posedge PCIE_RST) begin
        if (PCIE_RST) begin
            ...
            if (DN_FBUF_RD_TRIG) begin
                r_DMA_SIZE              <= DN_FBUF_RD_SIZE ; //buffer size
            end
        end
    end
    ...
    always @(posedge PCIE_CLK or posedge PCIE_RST) begin
        if (PCIE_RST) begin
            ...
            if (DN_FBUF_RD_TRIG) begin
                r_DMA_ADDR              <= DN_FBUF_RD_ADDR[63:0] ;  //address
//          end else if (s_DPK_TX_H1) begin
            end else if (r_DPK_TX_H1) begin
    ...
    always @(posedge PCIE_CLK or posedge PCIE_RST) begin
        if (PCIE_RST) begin
            ...
            if (r_DPK_TX_ACK) begin
                r_DMA_DCNT              <= r_DMA_SIZE[15:0] ;
            end else if (r_DMA_H1 == 1'b1 && (r_DMA_DCNT > 16'h80)) begin
                r_DMA_DCNT              <= r_DMA_DCNT - 16'h80 ;
            end else if (r_DMA_H1 == 1'b1 && (r_DMA_DCNT <= 16'h80)) begin
                r_DMA_DCNT              <= r_DMA_DCNT;
            end
        end
    end
    ...
    always @(posedge PCIE_CLK or posedge PCIE_RST) begin
        if (PCIE_RST) begin
            ...
            if (r_DMA_H1 == 1'b1 && (r_DMA_DCNT > 16'h80)) begin
                r_DMA_PSIZE             <= 10'h20 ;
            end else if (r_DMA_H1 == 1'b1 && (r_DMA_DCNT <= 16'h80)) begin
                r_DMA_PSIZE             <= r_DMA_DCNT[11:2];
            end
        end
    end
    ...
    always @(posedge PCIE_CLK or posedge PCIE_RST) begin
        if (PCIE_RST) begin
            ...
            r_DPK_TX_H0                 <= s_DPK_TX_H0 ;
            r_DPK_TX_H1                 <= s_DPK_TX_H1 ;
        end
    end
...
//          if (r_DMA_H1 == 1'b1 && (r_DMA_DCNT <= 16'h80)) begin
            if (s_DPK_TX_H1 == 1'b1 && (r_DMA_DCNT <= 16'h80)) begin
                r_DPK_TX_END            <= 1'b1 ;
            end else begin
                r_DPK_TX_END            <= 1'b0 ;
            end
...
//                                              {10'h20}                 // |      | --packet head payload length 128 byte = 32DW = 0x20 DW (Number in DW)
                                                {r_DMA_PSIZE}                 // |      | --packet head payload length 128 byte = 32DW = 0x20 DW (Number in DW)
                                                } ;

//          end else if (s_DPK_TX_H1 == 1'b1) begin
            end else if (r_DPK_TX_H1 == 1'b1) begin
                r_DPK_TX_DVLD               <=  1'b1                    ;
                r_DPK_TX_SOP                <=  1'b0                    ;
                r_DPK_TX_EOP                <=  1'b1                    ;
